...
Nous avons choisi plusieurs pondérations afin de donner un poids plus importants à certains critères. D'après l'étude réalisée dans la deuxième partie de ce compte-rendu, "Programmation linéaire multicritère", nous avons donc décider de privilégier le critère G1 et de minimiser le poids du critère G4 :
...
Nous avons décidé de priviléger le bénéfice au détriment d'une plus forte utilisation de la machine 4. En effet, en ces temps de crises, il est important que l'entreprise produise un chiffre d'affaire le plus élevé possible. \todo{(IL RESTE DES EXPLICATIONS A FOURNIR POUR CETTE PARTIE)}. 

\subsection{Simplification de la matrice de jugement}

De la même façon que dans l'étude \no1, la matrice de jugement est simplifiée en supprimant les projets 'f' et 'g' qui sont dominés respectivement par les projets 'e' et 'h'.

\subsection{Calcul de la matrice de concordance}

La matrice de concordance est calculée de la même façon que dans l'étude \no1. Cependant, certaines pondérations ont été ajoutées afin de donner plus de poids aux critères qui ont été jugés plus importants que les autres. Cela modifie donc la matrice de concordance. Lorsque le critère du projet i sera supérieur au même critère du projet j, nous allons donc compter cette valeur en pondérant par le coefficient associé au critère. 

Après calculs nous obtenons donc la matrice suivante : (devons-nous mettre tout le détail des calculs ? ou un exemple ?)
...
\todo{Faire une présentation avec 4 graphes et en légende les seuils de concordances et de discordances utilisés pour la représentation...
... 1er graphe : seuil de concordance = 3,75/4 ; seuil de discordance = 8/10...
... 2ème graphe : seuil de concordance = 3/4 ; seuil de discordance = 8/10...
... 3ème graphe : seuil de concordance = 3/4 ; seuil de discordance = 2/10...
... 4ème graphe : seuil de concordance = 2,5/4 ; seuil de discordance = 2/10...}

Pour la suite de cette étude, nous étudierons donc le graphe \todo{.... METTRE LE NUMERO DU GRAPHE A ETUDIER ....}

\todo{... IL RESTE A FAIRE L'ANALYSE DU GRAPHE QUI A ETE CHOISI POUR DETERMINER LE NOYAU ET DONC LE PROJET QUI EST LE MEILLEUR ! ...}

Après avoir analysé ces études, nous nous rendons compte que \todo{... IL FAUDRAIT LES RESULTATS DE L'ETUDE \no2 POUR CONTINUER LA CONCLUSION DE CETTE PARTIE ! ...}
